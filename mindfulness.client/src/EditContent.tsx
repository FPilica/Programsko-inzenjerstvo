...
function EditContent() {
  ...
  useEffect(() => {
    ...
    const contentToEdit = storedContent.find((item: ContentItem) => item.contentId === Number(id));
    ...
    if (contentToEdit) {
      ...
      setArticleText(contentToEdit.text || "");
    }
  }, [id]);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const storedContent = JSON.parse(localStorage.getItem("contentItems") || "[]");
    const updatedList = storedContent.filter((item: ContentItem) => 
      item.contentId !== Number(id)
    );
      ...
      <div className="background">
        <div className="addContentContainer">
          ...
          <div className="editContentHeader">
                <button className="myButton backButtonEdit" onClick={() => navigate(-1)}>Odustani</button>
                <button className="myButton deleteContentButton" onClick={handleDelete}>Izbriši</button>
            </div>
